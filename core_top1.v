...
assign fwd_ex_data  =exec_result_ex;   // connect EX forwarding later,// This taps the ALU result from the EX stage before it reaches the MEM stage


    assign fwd_mem_data = wb_data;   // WB forwarding path// This taps the ALU result from the EX stage before it reaches the MEM stage
...
id_ex_reg u_id_ex_reg (
    .clk            (clk),
    .rst_n          (rst_n),
    .stall          (stall_if_id), // From hazard unit
    .flush          (flush_id_ex), // From hazard/branch unit

    // Inputs from Decode/Control
    .pc_in          (pc_id),
 ...
 //   .imm_in         (imm_dec),
    ...
    .rd_addr_in     (rd_addr_dec),
        .exec_result_in   (exec_result_dec),
        .rs2_store_data_in(rs2_store_data_dec),
...
//    .imm_out        (imm_ex),
   ...
   .rs2_addr_out   (rs2_addr_ex),
    ...
    forward_unit u_forward_unit (
        // Execute stage sources (from ID/EX register)
        .ex_rs1_addr    (rs1_addr_ex), 
        .ex_rs2_addr    (rs2_addr_ex),

        // MEM stage destination (from MEM stage outputs)
        .exmem_rd_addr  (rd_addr_ex),
        .exmem_reg_write(reg_write_ex),

        // WB stage destination (from WB stage outputs)
        .memwb_rd_addr  (wb_rd_addr),
        .memwb_reg_write(wb_reg_write),

        // Outputs to MUXs in Execute Stage
